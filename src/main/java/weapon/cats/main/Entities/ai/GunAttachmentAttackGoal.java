...
public class GunAttachmentAttackGoal extends Goal{
    ...
    public GunAttachmentAttackGoal(LivingEntity mob, double mobSpeed, int intervalTicks, float maxShootRange) {
        this(mob, mobSpeed, intervalTicks, intervalTicks, maxShootRange);
    }

    public GunAttachmentAttackGoal(LivingEntity mob, double mobSpeed, int minIntervalTicks, int maxIntervalTicks, float maxShootRange) {
    ...
    public boolean canStart() {
    	if(mob.getEquippedStack(EquipmentSlot.CHEST).isOf(ItemManager.ATTACHER)) {
    		...
    		if(attachmentStack != null && attachmentStack.isOf(ItemManager.GUN_ATTACHMENT)) {
    ...
    public void tick() {
        ...
        if (--this.updateCountdownTicks <= 0) {
            ...
            GunAttachment.blastAt(mob, target);
            this.updateCountdownTicks = 1;
        }
    }
